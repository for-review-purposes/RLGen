...
transStateStructure([reputation_Inst_fl(_), gain_Inst_fl(_)]).
...
% to the next
init([reputation_Inst_fl(0), gain_Inst_fl(0)]).
...
restoreSitArg(reputation_Inst_fl(N),S,reputation_Inst_fl(N,S)).
restoreSitArg(gain_Inst_fl(N),S,gain_Inst_fl(N,S)).
